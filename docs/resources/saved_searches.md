...
- `schedule_priority` - (Optional) Raises the scheduling priority of the named search. Defaults to Default. On Splunk Enterprise this field is managed normally. On Splunk Cloud, set provider `ignore_schedule_priority = true` and omit this attribute from the resource; Splunk Cloud rejects create/update requests that include `schedule_priority`.
