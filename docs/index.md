...
* `ignore_schedule_priority` or `SPLUNK_IGNORE_SCHEDULE_PRIORITY` - (Optional) Omit `schedule_priority` from `splunk_saved_searches` create/update requests and skip syncing it on read. Defaults to `false` (Splunk Enterprise behavior). Set to `true` on Splunk Cloud when saved search updates fail with `schedule_priority is not supported by this handler`.
